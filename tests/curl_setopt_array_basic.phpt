--TEST--
curl_winssl_setopt_array() function - tests setting multiple cURL options with curl_winssl_setopt_array()
--CREDITS--
...
<?php
/*
 * Prototype:     bool curl_winssl_setopt_array(resource $ch, array $options)
...
// Start the test
echo '== Starting test curl_winssl_setopt_array($ch, $options); ==' . "\n";
...
curl_winssl_setopt_array($ch, $options);
$returnContent = curl_winssl_exec($ch);
...
?>
--EXPECT--
== Starting test curl_winssl_setopt_array($ch, $options); ==
